...
- Uçtan uca (E2E): `e2e/maestro/p0.yaml`, FIELD_TEST'in P0 sırası. CI (`.github/workflows/e2e.yml`) her push'ta değil: PR'da `e2e` etiketi varken (eklendiğinde ve etiket dururken her push'ta), elle (Actions → e2e → Run workflow) ve her gece main'de. Gradle ile x86_64 release APK'sı (yerel stack'e `http://10.0.2.2:54321`, OTA kapalı; Gradle önbelleğini gece koşusu yazar, diğerleri okur), emülatör, yerel Supabase ve functions serve. Koyu görünüm işi tasarım dalı (#13) merge edilene kadar kapalı. Ekran görüntüsü, video ve loglar artifact; ekran görüntüleri ve loglar ayrıca `e2e-screenshots` dalında (her koşu tek commit'lik orphan dalı force-push eder). İkinci masayı `scripts/e2e/bot-table.ts` oynar: `node --experimental-strip-types scripts/e2e/bot-table.ts setup` E2E mekanını kurar ve koordinatları yazar, `… serve` Maestro için 8787'de dinler. Bot yalnızca yerel stack'te ve dev projesinde çalışır, başka URL'i reddeder; dev projesinde yalnızca herkese açık Edge Function API'sini, publishable key'i ve test numarasını kullanır, veritabanı parolası ya da secret key istemez. Veritabanına doğrudan yazan eylemler (`setup`, `expire-turn`, `backdate-encounter`) yalnızca yerelde çalışır, dev URL'inde reddedilir. Yerelde koşmak için KVM'li bir emülatör ve Maestro CLI gerekir; komutlar workflow'daki adımların aynısı. Metin değişince akıştaki eşleşme de aynı PR'da güncellenir; aynı metin ekranda birden fazla görünüyorsa akış `testID` kullanır. Uygulamaya yalnızca `testID` eklenir.
- Karşı masa (tek telefonla P0, dev projesi): bot senin seçtiğin mekanda ikinci masa olarak bekler. Önce panelde (Authentication → Phone → test numaraları) bot için bir numara ekle, ör. `905550000002=<kod>`. Sonra repo kökünde:
  ```
...
- Edge Function'dan gelen her 5xx Sentry'ye `function` (ör. `tabu/mark`) ve `status` etiketleriyle gider; istek ve yanıt gövdesi gitmez. Yalnızca `_shared/pure/apiRetry.ts` → `IDEMPOTENT_CALLS` listesindeki çağrılar 5xx sonrası 500 ms bekleyip bir kez daha gönderilir; diğerleri (DM gönderme, istek, oda kurma…) asla tekrar gönderilmez. Ölçüt okuma değil idempotentliktir: iki kez gönderilince bir kez gönderilmiş gibi aynı durum ve yanıt. Her girdinin yanında onu iki kez gönderen entegrasyon testinin adı yazılıdır; `apiRetry.test.ts` bu testlerin var olduğunu denetler. Yeni girdi, böyle bir testle birlikte gelir.
...
3. Oyun sunucu otoriterdir: tur süresi (`ends_at`), kart dağıtımı, eylem yetkisi (kim ne zaman hangi eylemi yapabilir) ve skor sunucuda hesaplanır. Sesli Tabu'da doğruluğa masalar karar verir (Tabu yalnızca hakem masa, Pas yalnızca anlatan masa, Doğru ikisi de); her eylem bir kart indeksine bağlıdır, sunucu doğrular ve sırayı belirler, aynı karta ikinci eylem yok sayılır.
...
10. Kilit sırası: bir veritabanı işlemi satır kilitlerini (`for update`, `for no key update` ve `update`/`delete`'in aldığı örtük kilitler) her zaman bu sırayla alır: `profiles` → `table_sessions` → `rooms` → `join_requests` → odanın satırları (`tabu_turns`, `room_used_cards`, `reveal_decisions`, `messages`, `game_events`, `play_history`). İki hesap arasındaki işlerde: `play_history` → çift kilidi (`private.lock_pair`) → `friend_requests` → `friendships`, `mutual_friend_intents` → `dm_threads` → `dm_messages`, `dm_reads`. Masa oturumu yalnızca `for no key update` ile kilitlenir, `for update` ile asla (yabancı anahtar kontrolünün `key share`'ini engeller). Sıradan önce gelen bir satır sonradan gerekirse önce kilitsiz okunur, kilitler sırayla alınır, sonra satır kilit altında yeniden okunup doğrulanır (`rooms_respond`, `friends_respond`). Zamanlanmış işler, kullanıcı eylemlerinin kilitlediği tablolarda satırlarını `skip locked` ile seçer. Yeni tablo bu sıradaki yeriyle gelir; masa oturumu, oda ya da çift kilidi alan yeni bir yol, `supabase/tests/rooms.test.ts` ve `friends.test.ts` → `locks` kalıbında bir testle gelir.
