...
- Uçtan uca (E2E): `e2e/maestro/p0.yaml`, FIELD_TEST'in P0 sırası. CI'da `.github/workflows/e2e.yml` her PR'da çalışır: Gradle ile x86_64 release APK'sı (yerel stack'e `http://10.0.2.2:54321`, OTA kapalı), emülatör, yerel Supabase ve functions serve; açık ve koyu görünüm ayrı; ekran görüntüsü, video ve loglar artifact. İkinci masayı `scripts/e2e/bot-table.ts` oynar: `node --experimental-strip-types scripts/e2e/bot-table.ts setup` E2E mekanını kurar ve koordinatları yazar, `… serve` Maestro için 8787'de dinler. Bot yalnızca yerel stack'te ve dev projesinde çalışır, başka URL'i reddeder; saati ilerleten kancaları yalnızca yerelde. Yerelde koşmak için KVM'li bir emülatör ve Maestro CLI gerekir; komutlar workflow'daki adımların aynısı. Metin değişince akıştaki eşleşme de aynı PR'da güncellenir. Uygulamaya yalnızca `testID` eklenir.
